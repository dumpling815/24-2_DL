...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_Q, d_K, d_V, numhead, dropout):    
      super().__init__()  # 
      #Q1. Implement
      # d_model: feature vector size, int
      # Q,K,V -> typically passed as (d_model / numhead) from TF_Encoder_Block, int
      # numhead: # of head in multi head attention int
      # drop out: drop out probability, float
      ...
      # Linear layer after concatenation of multi-head results
      self.linear_out = nn.Linear(d_V * numhead, d_model)
    ...
    def forward(self, x_Q, x_K, x_V, src_batch_lens=None):      
      ...
      B, T, _ = x_Q.size()
        ...
        # Linear projections
        # view reshapes the tesnor without copying memory, similar to numpy's reshape()
        # 4 dimension, (batch, )
      Q = self.linear_Q(x_Q).view(B, T, self.numhead, self.d_Q).transpose(1, 2)
      K = self.linear_K(x_K).view(B, -1, self.numhead, self.d_K).transpose(1, 2)
      V = self.linear_V(x_V).view(B, -1, self.numhead, self.d_V).transpose(1, 2)

      # Scaled Dot-Product Attention
      attn_weights = torch.matmul(Q, K.transpose(-2, -1)) / self.scale  # QK^T
      if src_batch_lens is not None:
        mask = (torch.arange(T, device = src_batch_lens.device).expand(B, T) >= src_batch_lens.unsqueeze(1)).to(attn_weights.device)         
...
class TF_Encoder_Block(nn.Module):
    def __init__(self, d_model, d_ff, numhead, dropout):
        #Q3
        super().__init__()
        self.attention = MultiHeadAttention(d_model, d_model // numhead, d_model // numhead, d_model // numhead, numhead, dropout)
    ...
    def forward(self, x, src_batch_lens):
        ...
        attn_out = self.attention(x, x, x, src_batch_lens)
...
enc_d_model = 16 #Customize

enc_d_ff = 16 #Customize

enc_num_head = 4 #Customize

enc_num_layer= 4 #Customize
